window.onload = () => {
    ...
    const selectImageSize = document.querySelector("figure > select");
    ...
    selectSize.onchange = (event) => {
        const selectInstance = event.target;
        target.style.fontSize = selectInstance.selectedOptions[0].textContent.replace(" ",'');
    };

    selectImageSize.onchange = (event) => {
        const selectInstance = event.target;
        targetImage.style.height = selectInstance.selectedOptions[0].textContent.replace(" ",'');
    };
